...
\date{Working draft --- 9 August 2026}
...
known distractor positions.  A subsequent frozen shared-linear-policy test
passed only the separable-feature scenario and returned No-Go under partial or
complete feature aliasing.  The combined results support a narrow claim about
controlled policy movement and identify parameter coupling as the next
bottleneck; they do not establish causal credit assignment or neural-policy
performance.
\end{abstract}
...
\begin{enumerate}
    ...
    \item We run a second frozen transfer test with shared linear policy
    parameters.  It preserves the result only when pivotal and distractor
    features are separable, exposing an explicit limit of local reliability
    scaling followed by global parametric projection.
\end{enumerate}
...
The second frozen protocol replaces independent policy rows with shared linear
logits $\ell_k=\phi_k^\top W$.  Each method first forms a tabular OMD target and
then performs 60 fixed gradient steps minimizing average target-to-parametric
KL.  Three scenarios use separable, partially aliased, and completely aliased
position features.  Projected transfer required at least two of three scenarios
to remain within $0.02$ AUC and $75\%$ distractor KL of Projected Uniform.
...
\subsection{Frozen shared-parameter transfer}

\begin{table}[t]
\centering
\small
\caption{Frozen shared-linear-policy transfer.  Only the separable scenario
passes the declared AUC and distractor-KL conditions; the resulting transfer
decision is No-Go.}
\label{tab:function}
\begin{tabular}{lrrrcr}
\toprule
Feature geometry & Uniform AUC & Online AUC & $\Delta$AUC & KL ratio & Pass\\
\midrule
Separable         & 0.982550 & 0.980943 & -0.001608 & 0.193 & Yes\\
Partial aliasing  & 0.977562 & 0.977033 & -0.000529 & 0.967 & No\\
Complete aliasing & 0.971513 & 0.972722 &  0.001209 & 1.176 & No\\
\bottomrule
\end{tabular}
\end{table}

All three AUC conditions pass, but only the separable-feature task passes the KL
condition (Table~\ref{tab:function}).  The estimator itself continues to rank
pivotal positions above distractors: mean pivotal reliability is
$0.401$--$0.432$, mean distractor reliability is $0.137$--$0.167$, and top-$k$
precision is $0.704$--$0.725$.  The failure occurs after projection.  With
complete aliasing, every pivotal position shares its feature with a distractor,
so their projected KL movements are structurally identical.  The larger Online
base step then produces $17.6\%$ more distractor KL than Projected Uniform.  The
runtime ratio remains $0.999$--$1.055$ across the three scenarios.
...
The shared-linear transfer result sharpens the mechanism: the reliability
estimate can remain discriminative while the parameterization prevents its
local constraints from being realized.  Reliability must therefore enter the
parameter-space metric or constrained projection, rather than only the tabular
target distribution.

The current scope has five important limits.  First, function approximation is
restricted to fixed linear features; no representation is learned.  Second, known
...
Fifth, no nonlinear neural policy, language model, process reward model, or long
natural-language trajectory has been tested.  These limitations rule out a
direct language-model experiment with the current optimizer.
...
substantially less policy movement at known distractor decisions.  A frozen
shared-linear transfer test then showed that this behavior survives separable
features but not feature aliasing.  The next algorithmic stage is to make credit
reliability control parameter-space geometry itself, then subject that new
method to a separately frozen transfer protocol.
